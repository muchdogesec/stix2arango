...
class TestArangoDBQueries(BaseTestArangoDBQueries):
    ...
    def load_configuration(cls):
        ...
        cls.FILES = [
            dict(file="sigma-rule-bundle-condensed-original.json", ignore_embedded_relationships=False, stix2arango_note="test05"),
            dict(file="sigma-rule-bundle-condensed-update-1.json", ignore_embedded_relationships=False, stix2arango_note="test05"),
            dict(file="sigma-rule-bundle-condensed-update-2.json", ignore_embedded_relationships=False, stix2arango_note="test05"),
        ]
...
class TestVersionlessDBQueries(BaseTestArangoDBQueries):

    @classmethod
    def load_configuration(cls):
        super().load_configuration()
        cls.ARANGODB_DATABASE = "s2a_tests"
        cls.ARANGODB_COLLECTION = "test_v05"
        cls.FILES = [
            dict(file="sigma-rule-bundle-condensed-original.json", ignore_embedded_relationships=False, stix2arango_note="test_v05"),
            dict(file="sigma-rule-bundle-condensed-update-1.json", ignore_embedded_relationships=False, stix2arango_note="test_v05"),
            dict(file="sigma-rule-bundle-condensed-update-2.json", ignore_embedded_relationships=False, stix2arango_note="test_v05"),
        ]
        cls.VERSIONING_MODE = "versionless"


    def test_query_1(self):
        query = """
        RETURN LENGTH(
            FOR doc IN test_v05_vertex_collection
                FILTER doc._is_latest == true
                AND doc._stix2arango_note != "automatically imported on collection creation"
                RETURN doc
        )
    ...
    def test_query_2(self):
        ...
        """
        expected_result = [0] # Versionless does not keep old versions
        result = self.query_arango(query)
        self.assertEqual(result['result'], expected_result)
    ...
    def test_query_4(self):
        query = """
        RETURN LENGTH(
            FOR doc IN test_v05_edge_collection
                FILTER doc._is_latest == true
                AND doc._is_ref == false
                RETURN doc
        )
    ...
    def test_query_8(self):
        ...
        """
        expected_result = [0] # Versionless overwrites the embedded relationships too
        result = self.query_arango(query)
        self.assertEqual(result['result'], expected_result)
