...
def stix2arango_instance(mock_arango_service):
    ...
    with patch("stix2arango.stix2arango.stix2arango.config") as mock_config:
        ...
        with patch(
            "stix2arango.stix2arango.stix2arango.utils.load_file_from_url"
        ) as mock_load:
...
class TestPreUploadHooks:
    ...
    def test_pre_upload_hook_exception_logged_when_fail_on_error_false(
        self, stix2arango_instance, caplog
    ):
    ...
    def test_pre_upload_hook_exception_raises_when_fail_on_error_true(
        self, stix2arango_instance
    ):
...
class TestPostUploadHooks:
    ...
    def test_post_upload_hook_receives_correct_parameters(self, stix2arango_instance):
        ...
        stix2arango_instance._run_post_upload_hooks(
            "test_collection", test_objects, inserted_ids, existing_objects
        )
    ...
    def test_post_upload_hook_exception_logged_when_fail_on_error_false(
        self, stix2arango_instance, caplog
    ):
        """Test that hook exceptions are logged but don't stop processing when fail_on_error=False."""
        hooks_run = []
    ...
    def test_post_upload_hook_exception_raises_when_fail_on_error_true(
        self, stix2arango_instance
    ):
        """Test that hook exceptions stop processing when fail_on_error=True."""
        hooks_run = []
    ...
    def test_hooks_are_shared_across_instances(self, mock_arango_service):
        ...
        with patch("stix2arango.stix2arango.stix2arango.config") as mock_config:
            ...
            with patch(
                ...
                instance1 = Stix2Arango(
                    database="db1",
                    collection="col1",
                    file="file1.json",
                    create_collection=False,
                )
                instance2 = Stix2Arango(
                    database="db2",
                    collection="col2",
                    file="file2.json",
                    create_collection=False,
